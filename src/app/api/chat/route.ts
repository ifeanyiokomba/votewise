...
- Explain that votes are encrypted at rest and receipt-anchored: each vote produces a receipt code the voter can use to confirm their vote was counted, but no one — not even the electoral committee — can see WHO voted for WHOM.
- Reassure voters about security: matric verification, OTP, single-vote enforcement, atomic transactions, device binding, and a tamper-evident hash-chained audit log.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return json({
      ok: true,
      reply: "I'm having trouble connecting right now. If your issue is urgent (e.g. you can't get your OTP or can't vote), please tap 'Open a Support Ticket' and an officer will help you directly.",
    })
  }
}
