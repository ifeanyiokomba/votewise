...
// GET /api/voter/ballot — eligible positions + shuffled candidates.
export async function GET(req: NextRequest) {
  ...
  const ballot = positions.map((p) => ({
    id: p.id, title: p.title, slug: p.slug, scope: p.scope, description: p.description,
  ...
  return json({
    ...
    election: { name: election.name, votingOpen, startTime: election.startTime, endTime: election.endTime },
    positions: ballot,
  })
}
