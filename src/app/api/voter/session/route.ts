...
// GET /api/voter/session — check current voter session validity + return ballot summary.
...
// POST /api/voter/logout — clear the voter session token.
export async function POST(req: NextRequest) {
  const token = req.headers.get('x-voter-token') || req.headers.get('x-session-token') || req.headers.get('authorization')?.replace(/^Bearer\s+/i, '')
  if (token) {
    await db.voter.updateMany({ where: { sessionToken: token }, data: { sessionToken: null, sessionExpiresAt: null } })
  }
  return json({ ok: true })
}
