...
import { VoterProfile } from '@/components/votewise/voter-profile'
import { NavBar, Footer } from '@/components/votewise/shared'

function Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const sp = useSearchParams(); const org = sp.get('org') || undefined
  return <div className="flex min-h-screen flex-col"><NavBar /><main className="flex-1"><VoterProfile voterId={id} subdomain={org} /></main><Footer /></div>
}
